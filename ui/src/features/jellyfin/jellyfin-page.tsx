...
export function JellyfinPage() {
  ...
  return (
    <div className="flex h-full min-h-0 flex-col gap-3 overflow-auto p-3">
      <section className="rounded-[6px] bg-bg-2">
        <header className="flex flex-wrap items-center gap-3 bg-bg-4 px-3 py-2">
          ...
          <div className="ml-auto flex flex-wrap items-center gap-2">
            ...
            <button
              ...
              className="inline-flex items-center gap-2 rounded-[6px] bg-accent-err px-2.5 py-1.5 text-[12px] font-semibold text-fg-0 transition hover:opacity-90 disabled:opacity-40"
            >
      ...
      <section className="flex min-h-[22rem] flex-col overflow-hidden rounded-[6px] bg-bg-2">
        <header className="flex flex-wrap items-center gap-3 bg-bg-4 px-3 py-2">
          ...
          <button
            ...
            className="ml-auto inline-flex items-center gap-2 rounded-[6px] bg-bg-2 px-2.5 py-1.5 text-[12px] text-fg-1 transition hover:bg-[#343b41] hover:text-fg-0 disabled:opacity-40"
          >
        ...
        {!connection.isConfigured ? (
          <div className="flex flex-1 items-center justify-center bg-bg-0 p-6 text-sm text-fg-3">
          ...
          <div className="flex flex-1 items-center justify-center bg-bg-0 p-6 text-sm text-accent-err">
          ...
          <div className="min-h-0 flex-1 overflow-auto bg-bg-0">
            <div className="grid min-w-[48rem] grid-cols-[minmax(14rem,1fr)_minmax(16rem,1.3fr)_10rem_12rem] gap-3 bg-bg-4 px-3 py-2 font-mono text-[10px] uppercase tracking-label text-fg-3">
              <span>Name</span>
              <span>ID</span>
              <span>Collection</span>
              <span>Action</span>
            </div>
            {jellyfin.libraries.map((library, idx) => (
              <div
                key={library.id}
                className={
                  'grid min-w-[48rem] grid-cols-[minmax(14rem,1fr)_minmax(16rem,1.3fr)_10rem_12rem] items-center gap-3 px-3 py-3 ' +
                  (idx % 2 ? 'bg-bg-3' : 'bg-bg-2')
                }
              >
                <span className="text-sm font-semibold text-fg-0">{library.name}</span>
                <span className="break-all font-mono text-[12px] text-fg-2">{library.id}</span>
                <span className="font-mono text-[12px] text-fg-2">
                  {library.collectionType ?? 'unknown'}
                </span>
                {registeredConflicts.has(library.id) ? (
                  <span className="w-fit rounded-[3px] bg-accent-warn px-2 py-1 font-mono text-[10px] font-bold uppercase tracking-label text-bg-0">
                    already registered
                  </span>
                ) : (
                  <button
                    ...
                    className="inline-flex items-center justify-center rounded-[6px] bg-accent-live px-2.5 py-1.5 text-[12px] font-semibold text-bg-5 transition hover:bg-accent-live-hover"
                  >
      ...
      <Dialog
        ...
        onOpenChange={(open) => !open && setSelectedLibrary(null)}
      >
...
function InfoRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="grid grid-cols-[7rem_minmax(0,1fr)] gap-2">
      ...
      <span className="min-w-0 break-words text-fg-1">{value}</span>
    </div>
  );
}
    ...
    <div className="flex h-full items-center justify-center p-6">
      <div className="max-w-md rounded-[6px] bg-bg-2 p-5 text-center">
        ...
        <p className="mt-2 text-sm text-fg-2">
          Jellyfin settings require an authenticated session.
        </p>
      </div>
    </div>
  );
