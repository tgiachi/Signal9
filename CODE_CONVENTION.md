# Code Convention — SignalNine

This document defines coding conventions for the SignalNine project. It is intentionally strict to keep the codebase consistent and readable.
...
```
src/SignalNine.Core/Services/ConfigService.cs        → namespace SignalNine.Core.Services;
src/SignalNine.Service/Subscribers/SocketBroadcastSubscriber.cs → namespace SignalNine.Service.Subscribers;
tests/SignalNine.Tests/Core/EventBusServiceTests.cs  → namespace SignalNine.Tests.Core;
```
...
- When both Serilog and Microsoft.Extensions.Logging are in scope (e.g., `SignalNine.Service` which uses `Microsoft.NET.Sdk.Web`), add a using alias to resolve the ambiguity:
...
- All event types must implement `ISignalNineEvent`.
...
- Plugins implement `ISourcePlugin` (Id in reverse-domain format: `com.github.author.SignalNine.plugins.name`).
...
```
tests/SignalNine.Tests/<Domain>/<Subdomain>/<SubjectName>Tests.cs
namespace SignalNine.Tests.<Domain>.<Subdomain>;
```

Examples:
```
tests/SignalNine.Tests/Core/EventBusServiceTests.cs   → namespace SignalNine.Tests.Core;
tests/SignalNine.Tests/Service/UnixSocketServerTests.cs → namespace SignalNine.Tests.Service;
tests/SignalNine.Tests/Support/FakeSourcePlugin.cs    → namespace SignalNine.Tests.Support;
```
...
- Shared fakes, builders, and helpers go in `tests/SignalNine.Tests/Support/`.
- Do not mix reusable test infrastructure into domain test files.

### 13.4 InternalsVisibleTo

`SignalNine.Service.csproj` exposes internals to `SignalNine.Tests` via:
```xml
<InternalsVisibleTo Include="SignalNine.Tests"/>
```
